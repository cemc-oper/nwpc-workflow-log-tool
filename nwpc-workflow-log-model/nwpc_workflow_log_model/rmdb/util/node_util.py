...
class NodeUtil(object):
    ...
    def node_is_valid_from_session(
        cls, session, owner: str, repo: str, node_path: str, record_class: RecordBase
    ) -> bool:
        record_class.prepare(owner, repo)
        query = (
            session.query(record_class)
            .filter(record_class.node_path == node_path)
            .limit(1)
            .all()
        )
        if len(query) == 0:
    ...
    def sub_node_is_valid_from_session(
        ...
        path = node_path + "/"
        query = (
            session.query(record_class.node_path)
            .filter(
                "SUBSTR(record_fullname, 1, {path_length}) = '{path}'".format(
                    path_length=len(path), path=path
                )
            )
            .filter(
                "SUBSTR(record_fullname, 1, {alias_path_length}) != '{alias_path}'".format(
    ...
    @classmethod
    def get_node_type_from_session(
        cls, session, owner: str, repo: str, node_path: str, record_class: RecordBase
    ) -> str:
        ...
        if not cls.node_is_valid_from_session(
            session, owner, repo, node_path, record_class
        ):
            return node_type
        ...
        if cls.sub_node_is_valid_from_session(
            session, owner, repo, node_path, record_class
        ):
