...
class VersionUtil(object):
    ...
    @classmethod
    def add_new_repo_version(
        cls,
        session,
        repo: Repo,
        location: str,
        head_line: str,
        record_class: RecordBase,
    ) -> RepoVersion:
        version = RepoVersion()
        ...
        query = (
            session.query(RepoVersion)
            .filter(RepoVersion.repo_id == repo.repo_id)
            .filter(func.substr(RepoVersion.version_id, 1, 8) == version_date)
            .order_by(RepoVersion.version_id.desc())
        )
        ...
        version.version_name = (
            "version." + r.date.strftime("%Y-%m-%d") + "." + str(sub_version)
        )
    ...
    def get_version(
        ...
        version = (
            session.query(RepoVersion)
            .filter(RepoVersion.repo_id == repo.repo_id)
            .filter(RepoVersion.head_line == head_line)
            .first()
        )
        if version is None:
            logger.info("version not found, add a new version")
            version = cls.add_new_repo_version(
                session, repo, location, head_line, record_class
            )
        else:
